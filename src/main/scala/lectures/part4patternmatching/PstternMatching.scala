...
object PstternMatching extends App{
  ...
  //  using a guard along with case statement
  ...
  * */
  ...
  //Don't go for unnecessary pattern matching where its not required
  ...
  def show(e: Expr) : String = e match {
    ...
    case Prod(e1,e2) => {
      def maybeShowParentheseses(expr: Expr) = expr match {
        case Prod(_, _) => show(expr)
        case Number(_) => show(expr)
        case _         => "(" + show(expr) + ")"
      }
      maybeShowParentheseses(e1) +  " * " + maybeShowParentheseses(e2)
    }
